<template>
  <div class="res_top" ref="res_top">
    <div class="w">
      <div class="l logo">
        <span> <router-link to="/home" title="返回首页">调查船队管理系统</router-link> </span>
      </div>
      <div class="l nav">
  ...
  export default {
    data() {
      return {
        activeIndex: '1',
        tabs: [
          {"name": "综合管理", "path": "/resHome"},
          {"name": "调查船信息采集监控", "path": "/resDiaochachuan"},
          {"name": "调查船调度", "path": "/resInstrumenttation"},
          {"name": "调查船整改装", "path": "/resInstrumenttation1"},
          {"name": "志愿航次轨迹管理", "path": "/resZydcgl"},
          {"name": "志愿航次计划管理", "path": "/resHcjh"},
    ...
    mounted() {
      ...
      if (pathName == "/regionHome" || pathName == "/" || pathName == "/home" || pathName == '/obsRegionHome') {
        $(".res_top").hide()
      }
      if (pathName.substring(0, 4) == '/res') {
        $(".res_top").show()
      }
